...
# One Euro per keypoint, velocity clamping, confidence-weighted interpolation.
...
class PoseFilterPipeline:
    """One Euro smoothing, then velocity clamp, then confidence-weighted interpolation."""

    def __init__(self) -> None:
        self._filters = create_one_euro_keypoint_filters()
        self._previous: Optional[list[dict]] = None
        self._history: list[list[dict]] = []

    def reset(self) -> None:
        reset_one_euro_filters(self._filters)
        self._previous = None
        self._history.clear()
